            ...
            for Col in range (ColNb):
                if (WriteFile == 0):
                    ...
                    if (NormMatrix[Row,Col]> 0.0):
                        self.logger.info('Row {} ,  Col {}  value :{}'.format(Row,Col,NormMatrix[Row,Col]))
                elif (WriteFile == 1):
    ...
    def SCurveTakeOneStep(self,FilePath,RunNo,Prefix,FrameLength):
        ...
        if VResult < 0 :
            ...
            if VResult < 0 :
                ...
                # Acq polling
                VResult,GlobalValidSmpNb = self.Acq_Polling( GetDataBack = 0)
                if VResult < 0 :
                    self.logger.error('Error while starting the run:{}'.format(VResult))
                else:
                    # get norm data from file
                    self.logger.info('Acquisition completed, getting normalised data')
                    self.GetNormalisedDataFromFile(FilePath,Prefix,RunNo,WriteFile = 2)
