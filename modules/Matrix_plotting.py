...
def PlotMatrix2D(MatrixToPlot):
    ...
    VRowNb = MatrixToPlot.shape[0]
    #VColNb = np.size(MatrixToPlot,1)
    VColNb = MatrixToPlot.shape[1]
    logger.info("Plotting the matrix, matrix size :{:d},{:d}".format(VRowNb,VColNb))
    
    #plt.set_cmap('Greys')
    MatrixShow = PlotAx.matshow(MatrixToPlot)
    CBar = fig.colorbar(MatrixShow,cax=PlotAx)
    fig.canvas.draw()
    ...
    if (PlotType == 0):
        ...
        if platform.release() != 'XP':
            ...
            def plotSumOfFrame(change):
                ...
                #sum all the frames hits
                VSum = ct.c_int(0)
                Matrix2DSum = (MatrixToPlot.sum(2))/VTotalFrameNb
                # for VRowIndex in range (VRowNb):
                    # for VColIndex in range (VColNb):
                        # VSum.value = 0
                        # for VFrameIndex in range (FirstFrame,LastFrame+1):
